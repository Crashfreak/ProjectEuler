...
public class Problem11 extends EulerProblem {
    ...
    private int calculateMAX(int x, int y) {
        int value = grid[x][y];
        int currentMax = 0;
        // NW
        if (x > 2 && y > 2) {
            int NW = value * grid[x-1][y-1] * grid[x-2][y-2] * grid[x-3][y-3];
            currentMax = Math.max(NW, currentMax);
        }
        // N
        if (y > 2) {
            int N = value * grid[x][y-1] * grid[x][y-2] * grid[x][y-3];
            currentMax = Math.max(N, currentMax);
        }
        // NE
        if (y > 2 && x < 17) {
            int NE = value * grid[x+1][y-1] * grid[x+2][y-2] * grid[x+3][y-3];
            currentMax = Math.max(NE, currentMax);
        }
        // E
        if (x < 17) {
            int E = value * grid[x+1][y] * grid[x+2][y] * grid[x+3][y];
            currentMax = Math.max(E, currentMax);
        }
        // SE
        if (x < 17 && y < 17) {
            int SE = value * grid[x+1][y+1] * grid[x+2][y+2] * grid[x+3][y+3];
            currentMax = Math.max(SE, currentMax);
        }
        // S
        if (y < 17) {
            int S = value * grid[x][y+1] * grid[x][y+2] * grid[x][y+3];
            currentMax = Math.max(S, currentMax);
        }
        // SW
        if (x > 2 && y < 17) {
            int SW = value * grid[x-1][y+1] * grid[x-2][y+2] * grid[x-3][y+3];
            currentMax = Math.max(SW, currentMax);
        }
        // W
        if (x > 2) {
            int W = value * grid[x-1][y] * grid[x-2][y] * grid[x-3][y];
            currentMax = Math.max(W, currentMax);
        }
        return currentMax;
    }
}
